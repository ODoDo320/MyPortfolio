...
def ts_sum(df, window=10):
    ...
    return df.rolling(window, min_periods=0).sum()
...
def sma(df, window=10):
    ...
    :return: a pandas DataFrame with the time-series min over the past 'window' days.
    """
    return df.rolling(window, min_periods=0).mean()
...
def get_alpha(df):
        ...
        return df
# 2,3,5,6,7,(11),12~16,18~20,27,(30),33,(38),40~42,44,50,(51),53,54,60,83,101
class Alphas(object):
    def __init__(self, df_data):
        ...
        self.volume = df_data['volume']*1000
        # self.returns = df_data['S_DQ_PCTCHANGE'] 
        self.vwap = df_data['vwap']#(df_data['S_DQ_AMOUNT']*1000)/(df_data['volume']*1000+1) 
...
#                             columns=self.close.columns)
    ...
    def alpha027(self):
        ...
        alpha[alpha <= 0.5]=1
        return alpha  
        # cond = rank((ts_sum(correlation(rank(self.volume),rank(self.vwap), 6), 2)/2))
        # alpha = cond.notnull().astype(float).where(cond <= 0.5, -alpha)
        # return alpha
